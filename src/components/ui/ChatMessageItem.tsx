...
export function ChatMessageItem({ message: m, onSelectChoice, onCancelAction, onConfirmAction }: ChatMessageItemProps) {
  ...
  return (
    <div className={`w-full mb-3.5 ${isUser ? 'text-right' : 'text-left'}`}>
      ...
      {/* Pending Action Card — Prominent tactile pop-up card with Yes / No buttons */}
      {m.pendingAction && (
        <div className="mt-3 p-4 rounded-2xl bg-[#1a1e26] border border-[var(--color-accent)]/30 shadow-xl animate-in fade-in slide-in-from-bottom-2 duration-200">
          <div className="flex items-center gap-2.5 mb-2">
            <div className="w-7 h-7 rounded-lg bg-[var(--color-accent)]/15 flex items-center justify-center text-[var(--color-accent)]">
          ...
          {m.pendingAction.status === 'pending' && (
            <div className="flex items-center gap-2.5">
              <button
                ...
                className="flex-1 py-2.5 px-3 rounded-xl bg-[var(--color-accent)] text-black text-[13px] font-semibold flex items-center justify-center gap-1.5 shadow-md active:scale-95 hover:brightness-110 transition-all cursor-pointer"
              >
              ...
              <button
                ...
                className="py-2.5 px-4 rounded-xl bg-white/5 hover:bg-white/10 text-white/60 hover:text-white text-[13px] font-medium active:scale-95 transition-all cursor-pointer"
              >
                No, Cancel
              </button>
            </div>
          ...
          {m.pendingAction.status === 'confirmed' && (
            <div className="flex items-center gap-1.5 text-[12px] font-medium text-[var(--color-accent)] bg-[var(--color-accent)]/10 px-3 py-1.5 rounded-lg w-fit">
          ...
          {m.pendingAction.status === 'cancelled' && (
            <div className="text-[12px] font-medium text-white/40 bg-white/5 px-3 py-1.5 rounded-lg w-fit">
              Cancelled
            </div>
          )}
        </div>
