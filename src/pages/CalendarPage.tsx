...
export function CalendarPage({ onBack }: PageProps) {
  ...
  return (
    <div className="placeholder-page flex flex-col overflow-hidden">
      {/* Header */}
      ...
      {authStatus.connected && (
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="text-center">
            <button onClick={goToday} className="text-[17px] font-semibold text-white tracking-tight hover:text-[var(--color-accent)] transition-colors cursor-pointer">
      ...
      <div className="flex-1 overflow-y-auto px-4 scrollbar-hide">
        ...
        {!loadingStatus && !authStatus.connected && (
          <div className="flex flex-col items-center justify-center h-full text-center px-6">
            <div className="w-14 h-14 rounded-2xl bg-[var(--color-surface)] flex items-center justify-center mb-4 border border-white/[0.05]">
            ...
            <button onClick={handleConnect} className="px-6 py-3 rounded-2xl bg-[var(--color-accent)] text-[#12151a] text-[14px] font-semibold active:scale-95 transition-transform cursor-pointer shadow-lg shadow-[var(--color-accent)]/10">
              Sign in with Google
            </button>
          </div>
        )}
          ...
          <div className="pb-6">
            ...
            {allDayEvents.length > 0 && (
              <div className="mb-4 flex flex-col gap-1.5">
                {allDayEvents.map((evt) => (
                  <div key={evt.id} className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-2xl bg-white/[0.03] border border-white/[0.05]">
                    <div className="w-1.5 h-4 rounded-full" style={{ backgroundColor: getEventColor(evt.colorId) }} />
            ...
            {loadingEvents ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <div className="w-10 h-10 rounded-full bg-white/[0.04] flex items-center justify-center mb-3 text-white/30 border border-white/[0.05]">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                    <line x1="16" y1="2" x2="16" y2="6" />
                    <line x1="8" y1="2" x2="8" y2="6" />
              ...
              <div className="flex flex-col gap-2">
                {timedEvents.map((evt) => (
                  <div key={evt.id} className="p-3.5 rounded-2xl bg-white/[0.03] border border-white/[0.05] flex items-center justify-between gap-3">
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-[60] flex flex-col justify-end">
          ...
          {/* Modal Container */}
          <div className="relative w-full max-w-[430px] mx-auto bg-[#181c24] border-t border-white/[0.08] rounded-t-[32px] p-6 shadow-2xl animate-in slide-in-from-bottom duration-250 pb-8 max-h-[85vh] overflow-y-auto scrollbar-hide">
            ...
            <div className="mb-5">
              ...
              <div className="p-1 bg-black/40 border border-white/[0.06] rounded-2xl flex gap-1">
                <button
                  onClick={() => { sensory.tapLight(); setViewMode('timeline'); }}
                  className={`flex-1 py-2 rounded-xl text-[13px] font-medium transition-all text-center cursor-pointer ${
                    viewMode === 'timeline'
                      ? 'bg-[var(--color-accent)] text-[#12151a] font-semibold shadow-sm'
            ...
            <div className="mb-5">
              ...
              <div className="bg-black/30 border border-white/[0.06] rounded-2xl overflow-hidden divide-y divide-white/[0.04]">
                {/* Sync Button */}
                <button
                  onClick={handleManualSync}
                  disabled={isSyncing}
                  className="w-full py-3.5 px-4 text-white text-[13.5px] font-medium flex items-center justify-between hover:bg-white/[0.03] active:bg-white/[0.06] transition-all cursor-pointer"
                >
                  <span className="flex items-center gap-3">
                    <div className="w-7 h-7 rounded-xl bg-[var(--color-accent)]/10 text-[var(--color-accent)] flex items-center justify-center">
                      <svg
                        width="15"
                        height="15"
                        viewBox="0 0 24 24"
                ...
                {!confirmClearDay ? (
                  <button
                    ...
                    className="w-full py-3.5 px-4 text-white/80 hover:text-white text-[13.5px] font-medium flex items-center justify-between hover:bg-white/[0.03] active:bg-white/[0.06] transition-all cursor-pointer"
                  >
                    <span className="flex items-center gap-3">
                  ...
                  <div className="p-3.5 bg-red-500/[0.08]">
                    <p className="text-[12.5px] text-red-200 mb-2.5 pl-0.5">Delete ALL events on {formatDateFull(selectedDate)}?</p>
                    <div className="flex gap-2">
                      <button
                        onClick={handleClearSelectedDay}
                        className="flex-1 py-2 rounded-xl bg-red-500 text-white text-[12px] font-semibold active:scale-95 transition-all cursor-pointer shadow-sm"
                      >
                        Yes, Clear Day
                      </button>
                      <button
                        onClick={() => setConfirmClearDay(false)}
                        className="py-2 px-4 rounded-xl bg-white/10 text-white/70 hover:text-white text-[12px] font-medium active:scale-95 transition-all cursor-pointer"
                      >
                        Cancel
                      </button>
                    </div>
            ...
            <div className="pt-2">
              ...
              <div className="bg-black/30 border border-white/[0.06] rounded-2xl p-3.5 mb-3 flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <div className="w-2 h-2 rounded-full bg-[var(--color-accent)] shadow-[0_0_8px_rgba(93,202,165,0.6)]" />
                  <span className="text-[13px] text-white/85 font-medium">Life OS Dedicated Calendar</span>
                </div>
              ...
              {!confirmDisconnect ? (
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-red-500/[0.08] hover:bg-red-500/[0.13] text-red-400 text-[13px] font-medium flex items-center justify-center gap-2 border border-red-500/15 active:scale-[0.99] transition-all cursor-pointer"
                >
                ...
                <div className="p-3.5 rounded-2xl bg-red-500/[0.08] border border-red-500/20">
                  <p className="text-[13px] text-red-200 font-medium mb-1">Disconnect Google Account?</p>
                  <p className="text-[11.5px] text-red-300/60 mb-3">You will need to sign in again to sync events.</p>
                  <div className="flex gap-2">
                    <button
